...
transpose:
    ...
    li   t0, 0                      # j = 0
column_loop:
    bge  t0, a3, done               # if j >= M, exit
    li   t1, 0                      # i = 0
row_loop:
    bge  t1, a2, next_column         # if i >= N, move to next column
    ...
    slli t4, a3, 2                   # t4 = M * 4 (offset in bytes)
    vmul.vx v4, v4, t4               # v4 = v4 * (M * 4)

    # calc offset
    mul  t5, t1, a3                  # t5 = i * M (line start pos)
    add  t5, t5, t0                  # t5 = i * M + j (offset)
    ...
    # calc offset
    mul  t5, t0, a2                  # t5 = j * N (line start pos)
    add  t5, t5, t1                  # t5 = j * N + i (offset)
    slli t5, t5, 2                   # t5 *= 4 (offset in bytes)
